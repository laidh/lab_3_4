...
class TestShopManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.producer = Product("producer", 1, 1000, 5, "Dobryana")
        self.shakes = Drinks("Triple Milkshake", 3, 800, Drink.MILK, 2)

        all_objects = [self.producer, self.shakes, self.dessert]
    ...
    def test_sort_by_number_of_dishes(self):
        self.assertEqual(
            ...
            [self.shakes, self.dessert, self.producer]
        )

    def test_sort_by_caloric(self):
        self.assertEqual(
            self.manager.sort_by_caloric(SortOrder.ASC),
            [self.shakes, self.producer, self.dessert]
        )
